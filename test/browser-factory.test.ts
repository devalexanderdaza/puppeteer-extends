...
describe("BrowserFactory", () => {
  ...
  it("should handle launch errors", async () => {
    (puppeteerExtra.launch as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
      new Error("Failed to launch browser"),
    );

    await expect(BrowserFactory.getBrowser({ isDebug: true })).rejects.toThrow(
  ...
  it("should create separate instances with different IDs", async () => {
    const browser1 = await BrowserFactory.getBrowser({
      instanceId: "instance1",
    });
    const browser2 = await BrowserFactory.getBrowser({
